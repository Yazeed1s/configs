...
# fastfetch(ignored): neofetch for sys-info
...
pacman_packages=(
	...
	btop nemo alacritty
	firefox neovim vim
	nitrogen brightnessctl gvfs
	bluez bluez-utils lxappearance
	go rust cmake
	clang grpc protobuf
	pavucontrol openssh dmidecode
	zsh amd-ucode arandr
	pulseaudio alsa-utils xf86-video-qxl
	xorg xorg-xinit pacman-contrib
	git mesa base-devel
...
read -n1 -rep 'would you like to start SSH server? (y,n)' ANS
if [[ $ANS == "Y" || $ANS == "y" ]]; then
...
# Start MySQL (MariaDB)
read -n1 -rep 'setup a MySQL (MariaDB) (y,n)' ANS
if [[ $ANS == 'Y' || $ANS == 'y' ]]; then
...
# Start PostgreSQL
read -n1 -rep 'setup a PostgreSQL (y,n)' ANS
if [[ $ANS == 'Y' || $ANS == 'y' ]]; then
